...
//nvcc -ptx "E:\семестр 7\НИР\kernel_Geom.cu" -ccbin "C:\Program Files (x86)\Microsoft Visual Studio\2017\Community\VC\Tools\MSVC\14.16.27023\bin\Hostx64\x64" -gencode arch=compute_35,code=sm_35 -rdc=true
...
__device__ bool ComputeIntersection(float4 * Edges, const unsigned int N, float2 pos, float2 direction, float2 &newpos, float2 &newdirection, float &r,const float s_r, const float x_s, const float y_s){
    ...
    float h = b*b - c;
    if( h>=0.0 && b<=0){ // intersection with source
        h = sqrt(h);
        minR = -b-h;
        hitSource = true;
    }
    ...
    if (minR<1.0e+35){
        ...
        newdirection = direction-2*dot(n,direction)*n;
        newdirection = normalize(newdirection);
        newpos = pos+(1-eps)*minR*direction;// + newdirection*eps;
        return false;
    }
    r=-1.0;
    return false;
...
__global__ void kernelRayTracing(const float *Edges_f, const unsigned int N, float *IR, const unsigned int SampleCount,const int maxReflections,const int T,const float v,const float d_t,const float x_ir,const float y_ir, const float x_s,const float y_s, const float s_r=0.1)
{
    ...
    float phi = 2*float(i)*pi/SampleCount;
